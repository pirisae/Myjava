...
//        Beverage05.STORE_NAME =" ggg"
        System.out.println("어서 오세요~~" + Beverage05.STORE_NAME);
        ...
        System.out.println("향상 for를 사용한 출력"); // for(타입 단수 : 복수){}
        for (Beverage05 item : orderList) {
            ...
            if (item instanceof Americano05) {
                ...
                ((Espresso05) item).shotaddable(2);
                }else if(item instanceof Latte05) {
                ((Latte05) item).changeMilk("초코 우유");

                }else if (item instanceof SpecialCoffee05){
                ((SpecialCoffee05)item).shotaddable(1);
                ((SpecialCoffee05)item).adjustwater(60);
                ((SpecialCoffee05)item).changeMilk("qkqksk 우유");
            }else if (item instanceof Cappuccino05){
                ((Cappuccino05)item).changeMilk("아몬드 우유");
